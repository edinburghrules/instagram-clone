...
describe("<Dashboard />", () => {
  ...
  it("renders the dashboard with a user profile and follows a user from the suggested profile, it also likes a photo, posts a comment and signs out", async () => {
    await act(async () => {
      ...
      await waitFor(async () => {
        ...
        expect(
          screen.getAllByAltText("Instagram")
        ).toBeTruthy();
        expect(
          screen.getByAltText("steve profile")
        ).toBeTruthy();
        expect(
          screen.getAllByText("Saint George and the Dragon")
        ).toBeTruthy();
        expect(
          screen.getByText("Suggested for you")
        ).toBeTruthy();
        ...
        fireEvent.click(
          screen.getByTestId(
            "photo-liked-494LKmaF03bUcYZ4xhNu"
          )
        );
        ...
        fireEvent.click(
          screen.getByTestId(
            "focus-input-494LKmaF03bUcYZ4xhNu"
          )
        );
        ...
        fireEvent.change(
          screen.getByTestId(
            "add-comment-494LKmaF03bUcYZ4xhNu"
          ),
        ...
        expect(
          screen.getByText("Amazing photo!")
        ).toBeTruthy();
      });
  ...
  it("renders the dashboard with a user profile that has no photos", async () => {
    await act(async () => {
      usePhotos.mockImplementation(() => ({ photos: [] }));
      getSuggestedProfiles.mockImplementation(
        () => suggestedProfilesFixture
      );
      useUser.mockImplementation(() => ({
